...
from src.math_utils import ec_mul, mul_mod_Q
...
func computeChallenges{output_ptr : felt*, range_check_ptr,
                      bitwise_ptr : BitwiseBuiltin*}(part1 : Part1, k : felt)
                      -> (){
    ...
    let (challenge) = computeBaseChallenge(part1.cy, 
                                           part1.cts_size, part1.cts, 
                                           part1.css_size, part1.css,
                                           part1.cqs_size, part1.cqs);


    let (res) = mul_mod_Q(challenge, challengeNeg1);
    serialize_word(res);
...
// Test for computeChallenges
func main{output_ptr : felt*, range_check_ptr, bitwise_ptr : BitwiseBuiltin*}(){
    alloc_locals;
    
    let (local cts : EcPoint*) = alloc();
    let (local css : EcPoint*) = alloc();
    let (local cqs : EcPoint*) = alloc();
    local cy : EcPoint = EcPoint(7474, 84848);
    let (local eInvs : felt*) = alloc();

    assert cts[0] = EcPoint(2, 3);
    assert cts[1] = EcPoint(5, 7);
    assert css[0] = EcPoint(1, 1);
    assert cqs[0] = EcPoint(4, 8);
    assert cqs[1] = EcPoint(1234, 356);
    assert cqs[2] = EcPoint(6, 35);
    assert eInvs[0] = 1;
    assert eInvs[1] = 2;

    local part1 : Part1 = Part1(cy, cts, css, cqs, eInvs, 2, 1, 3, 2);
    local k = 8;

    computeChallenges(part1 = part1, k = k);
